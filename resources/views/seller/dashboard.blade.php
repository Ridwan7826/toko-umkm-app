<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-end mb-4">
                <a href="{{ route('dashboard', ['refresh' => 1]) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition text-sm font-medium shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                    ...
                    <p class="text-2xl font-bold text-emerald-600 mt-2">Rp {{ number_format($totalRevenue30Days, 0, ',', '.') }}</p>
                </div>
                <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                    <h3 class="text-sm font-medium text-slate-500 uppercase">Rata-rata Nilai Pesanan</h3>
                    <p class="text-2xl font-bold text-blue-600 mt-2">Rp {{ number_format($aov, 0, ',', '.') }}</p>
                </div>
                <div class="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                    <h3 class="text-sm font-medium text-slate-500 uppercase">Tingkat Pembatalan</h3>
                    <p class="text-2xl font-bold text-red-600 mt-2">{{ number_format($cancelRate, 1, ',', '.') }}%</p>
                </div>
            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Pie Chart: Order Status
            const orderStatusData = @json($orderStatusComposition);
